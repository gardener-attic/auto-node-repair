...
https://github.com/kubernetes/autoscaler/blob/cluster-autorepair-1.0.0/cluster-autoscaler/util/kubernetes/listers.go
*/
...
func NewListerRegistry(allNode NodeLister, readyNode NodeLister, notReadyNode NodeLister, scheduledPod PodLister,
	...
	return listerRegistryImpl{
		...
		notReadyNodeLister:        notReadyNode,
...
func NewListerRegistryWithDefaultListers(kubeClient client.Interface, stopChannel <-chan struct{}) ListerRegistry {
	...
	notReadyNodeLister := NewNotReadyNodeLister(kubeClient, stopChannel)
	...
	return NewListerRegistry(allNodeLister, readyNodeLister, notReadyNodeLister, scheduledPodLister,
...
// ReadyNodeLister returns the ReadyNodeLister registered to this registry
func (r listerRegistryImpl) NotReadyNodeLister() NodeLister {
	return r.notReadyNodeLister
}
...
func (notReadyNodeLister *NotReadyNodeLister) List() ([]*apiv1.Node, error) {
	nodes, err := notReadyNodeLister.nodeLister.List(labels.Everything())
	if err != nil {
		return []*apiv1.Node{}, err
	}
	notReadyNodes := make([]*apiv1.Node, 0, len(nodes))
	for _, node := range nodes {
		if !IsNodeReadyAndSchedulable(node) {
