...
https://github.com/kubernetes/autoscaler/blob/cluster-autoscaler-release-1.3/cluster-autoscaler/cloudprovider/azure/azure_cache.go
*/
...
func (m *asgCache) FindForInstance(instance *azureRef, vmType string) (cloudprovider.NodeGroup, error) {
	...
	if vmType == vmTypeVMSS {
		...
		if ok := virtualMachineRE.Match([]byte(instance.Name)); ok {
			glog.V(3).Infof("Instance %q is not managed by vmss, omit it in node repair", instance.Name)
	...
	if vmType == vmTypeStandard {
		...
		if ok := virtualMachineRE.Match([]byte(instance.Name)); !ok {
			glog.V(3).Infof("Instance %q is not in Azure resource ID format, omit it in node repair", instance.Name)
			m.notInRegisteredAsg[*instance] = true
			return nil, nil
		}
	}
