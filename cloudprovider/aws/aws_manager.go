...
https://github.com/kubernetes/autoscaler/blob/cluster-autorepair-1.0.0/cluster-autoscaler/cloudprovider/aws/aws_manager.go
*/
...
func extractLabelsFromAsg(tags []*autoscaling.TagDescription) map[string]string {
	...
	for _, tag := range tags {
		k := *tag.Key
		v := *tag.Value
		splits := strings.Split(k, "k8s.io/cluster-autorepair/node-template/label/")
		if len(splits) > 1 {
			label := splits[1]
			if label != "" {
				result[label] = v
...
func extractTaintsFromAsg(tags []*autoscaling.TagDescription) []apiv1.Taint {
	...
	for _, tag := range tags {
		k := *tag.Key
		v := *tag.Value
		splits := strings.Split(k, "k8s.io/cluster-autorepair/node-template/taint/")
		if len(splits) > 1 {
