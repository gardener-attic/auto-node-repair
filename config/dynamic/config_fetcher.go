...
https://github.com/kubernetes/autoscaler/blob/cluster-autorepair-1.0.0/cluster-autoscaler/config/dynamic/config_fetcher.go
*/
...
func (c *configFetcherImpl) FetchConfigIfUpdated() (*Config, error) {
	...
	if err != nil {
		c.recorder.Eventf(cm, apiv1.EventTypeNormal, "FailedToBeLoaded",
			"cluster-autorepair tried to load this configmap but failed: %v", err)
